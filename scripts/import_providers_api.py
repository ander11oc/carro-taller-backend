"""
Import CloudFleet providers through /api/v1/fleet/providers/import-csv.
"""
import argparse
import io
import sys
...
def find_csv() -> Path:
    ...
    candidates = sorted(workspace.glob("*Proveedores*.csv"))
    if candidates:
        return candidates[-1]
    raise FileNotFoundError("No se encontro el CSV de proveedores")
...
def import_providers(api: str, token: str, csv_path: Path) -> None:
    headers = {"Authorization": f"Bearer {token}"}
    print(f"Archivo: {csv_path}")
    print("Enviando CSV al importador de proveedores...")

    with csv_path.open("rb") as file:
        response = requests.post(
            f"{api.rstrip('/')}/api/v1/fleet/providers/import-csv",
            headers=headers,
...
def main() -> None:
    ...
    import_providers(args.api, token, csv_path)
